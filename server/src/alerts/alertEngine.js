...
function deepLinkFor(symbol) {
  ...
  return `${base}/index.html?symbol=${encodeURIComponent(symbol)}#companyOverview`;
}
...
async function record(pool, payload) {
  try {
    await pool.query(
      ...
      [payload.ownerKey, payload.scannerId, payload.symbol, payload.tradeDate, payload.channel, payload.status, payload.error || null, payload.deepLink]
    );
  } catch (error) {
...
async function runAlertPipeline(pool, scannerId, matches) {
  ...
  let fresh;
  try {
    fresh = await detectNewMatches(pool, scannerId, matches.map(match => ({ symbol: match.symbol, tradeDate: match.tradeDate })));
  } catch (error) {
  ...
  for (const preference of preferences) {
    for (const detected of fresh) {
      const match = { ...detected, ...(bySymbol.get(detected.symbol) || {}) };
      ...
      if (preference.email_enabled) {
        try {
          ...
          await record(pool, { ownerKey: preference.owner_key, scannerId, symbol: match.symbol, tradeDate: match.tradeDate, channel: 'email', status: 'FAILED', error: error.message, deepLink });
          summary.failed += 1;
        }
      }
      ...
      if (preference.push_enabled) {
        let subscriptions = [];
        try {
          subscriptions = (await pool.query('SELECT * FROM push_subscriptions WHERE owner_key=$1 AND invalidated_at IS NULL', [preference.owner_key])).rows;
        } catch (error) {
        ...
        for (const subscription of subscriptions) {
          try {
            const result = await push.send(
              ...
              push.buildAlertPayload({ symbol: match.symbol, verdict: match.verdict, score: match.score, scannerId, deepLink })
            );

            await record(pool, { ownerKey: preference.owner_key, scannerId, symbol: match.symbol, tradeDate: match.tradeDate, channel: 'push', status: result.status, error: result.error, deepLink });
            result.status === 'SENT' ? summary.sent += 1 : summary.failed += 1;

            if (result.invalid) {
              await pool.query('UPDATE push_subscriptions SET invalidated_at=now() WHERE id=$1', [subscription.id]);
            }
          } catch (error) {
            await record(pool, { ownerKey: preference.owner_key, scannerId, symbol: match.symbol, tradeDate: match.tradeDate, channel: 'push', status: 'FAILED', error: error.message, deepLink });
            summary.failed += 1;
          }
        }
      }
