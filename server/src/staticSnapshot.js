...
async function fetchFo(date) {
  ...
  requireColumns(rows, 'NSE F&O UDiFF', ['TckrSymb', 'Sgmt', 'FinInstrmTp', 'XpryDt', 'OpnIntrst', 'ChngInOpnIntrst']);
  const futures = rows
    ...
    .map(r => ({ symbol: clean(r.TckrSymb), trade_date: formatYmd(date), expiry: clean(r.XpryDt), close: num(r.ClsPric), oi: num(r.OpnIntrst), change_oi: num(r.ChngInOpnIntrst), available: true }))
    .filter(r => r.symbol && r.expiry);
  if (!futures.length) throw new Error(`NSE F&O archive parsed successfully but produced 0 stock/index futures rows for ${formatYmd(date)}`);
  return futures;
}
...
function materialize(historySnapshots) {
  const historyBySymbol = new Map(); const futuresBySymbolDate = new Map(); const derivativesSymbols = new Set();
  for (const day of historySnapshots) {
    ...
    for (const row of day.futures || []) { const key = `${row.symbol}|${row.trade_date}`; derivativesSymbols.add(String(row.symbol || '').toUpperCase()); const existing = futuresBySymbolDate.get(key); if (!existing || String(row.expiry) < String(existing.expiry)) futuresBySymbolDate.set(key, row); }
  }
  for (const rows of historyBySymbol.values()) rows.sort((a, b) => a.trade_date.localeCompare(b.trade_date));
  return { results: buildScannerResults(historyBySymbol, futuresBySymbolDate, derivativesSymbols), periods: buildPeriodResults(historyBySymbol, futuresBySymbolDate, derivativesSymbols), derivativesSymbols };
}
function buildCurrentDetection(historySnapshots, currentResults) {
  ...
  for (const day of historySnapshots) {
    ...
    for (const row of day.futures || []) { const symbol = String(row.symbol || '').toUpperCase(); derivativesSymbols.add(symbol); const key = `${symbol}|${row.trade_date}`; const existing = futures.get(key); if (!existing || String(row.expiry) < String(existing.expiry)) futures.set(key, row); }
  }
  for (const rows of bySymbol.values()) rows.sort((a, b) => String(a.trade_date).localeCompare(String(b.trade_date)));
  const out = new Map();
  for (const result of currentResults || []) {
    if (result.verdict !== 'ACCUMULATION CONFIRMED') continue; const symbol = String(result.symbol || '').toUpperCase(); const rows = bySymbol.get(symbol) || []; if (!rows.length) continue;
...
async function main() {
  ...
  const latestSymbols = new Set((latest.cm || []).map(row => String(row.symbol || '').toUpperCase())); const currentResults = materialized.results.filter(row => String(row.tradeDate || '') === latest.tradeDate && latestSymbols.has(String(row.symbol || '').toUpperCase()));
  const detection = buildCurrentDetection(history, currentResults); const addNames = rows => rows.map(row => ({ ...row, companyName: universe.get(row.symbol) || null, indexMembership: indexUniverses ? membershipFor(row.symbol, indexUniverses.memberships) : null, detection: detection.get(String(row.symbol || '').toUpperCase()) || null }));
  const tradingDays = history.map(day => day.tradeDate).filter(Boolean); const snapshot = { status: 'ok', dataStatus: 'EOD VERIFIED', asOf: latest.tradeDate, generatedAt: new Date().toISOString(), source: 'NSE EOD public archives', universeSource: NSE_EQUITY_UNIVERSE, indexUniverseStatus: indexUniverses?.status || 'DATA N/A', indexUniverseSources: indexUniverses?.sources || {}, historyDays: history.length, tradingDays, results: addNames(currentResults), periods: Object.fromEntries(Object.entries(materialized.periods).map(([key, rows]) => [key, addNames(rows)])) };
