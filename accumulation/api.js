(function (root) {
  ...
  // No conditions, numeric inputs or Apply button.
  window.addEventListener('DOMContentLoaded',()=>setTimeout(()=>{
    ...
    function apply(){
      [...tbody.rows].forEach(row=>{
        let visible=true;
        const price=num(text(row,1));
        const prev=num(text(row,2));
        ...
        if(state.price==='positive'&&!(price!==null&&price>0))visible=false;
        ...
        if(state.score==='high'&&!(score!==null&&score>=70))visible=false;
        if(state.score==='low'&&!(score!==null&&score<50))visible=false;
        ...
        if(state.delivery==='strong'&&!(delivery!==null&&delivery>=50))visible=false;
      ...
      if(summary){summary.textContent=`Showing ${[...tbody.rows].filter(r=>r.style.display!=='none').length.toLocaleString()} stocks • Filters: ${Object.keys(state).length?'active':'none'}`;}
    }
    ...
    function optionsFor(key){
      if(key==='stock')return [['All','all'],['A–Z','sort-stock-asc'],['Z–A','sort-stock-desc']];
      if(key==='price')return [['All','all'],['Low to High','sort-asc'],['High to Low','sort-desc']];
      if(key==='prevClose')return [['All','all'],['Low to High','sort-asc'],['High to Low','sort-desc']];
      if(key==='priceChange')return [['All','all'],['Positive Only','positive'],['Negative Only','negative']];
      if(key==='score')return [['All','all'],['Highest First','sort-desc'],['Lowest First','sort-asc'],['70+ Scores','high'],['Below 50','low']];
      ...
      if(key==='volumeRatio')return [['All','all'],['Highest First','sort-desc'],['Lowest First','sort-asc'],['1x or More','strong'],['Below 1x','weak']];
      if(key==='delivery')return [['All','all'],['Highest First','sort-desc'],['Lowest First','sort-asc'],['50% or More','strong'],['Below 50%','low']];
      if(key==='obv')return [['All','all'],['Highest First','sort-desc'],['Lowest First','sort-asc'],['Positive OBV','positive'],['Negative OBV','negative']];
      if(key==='changeOi')return [['All','all'],['Highest First','sort-desc'],['Lowest First','sort-asc'],['OI Increasing','up'],['OI Decreasing','down'],['OI Not Available','na']];
      return [['All','all']];
    }
    function open(button,key){
      ...
      optionsFor(key).forEach(([label,value])=>{
        ...
        item.addEventListener('click',()=>{
          ...
          else if(value==='sort-asc'||value==='sort-desc'||value==='sort-stock-asc'||value==='sort-stock-desc') delete state[key];
          else state[key]=value;
          close();
          if(value==='sort-stock-asc'||value==='sort-stock-desc') sortSymbols(value==='sort-stock-asc'?'asc':'desc');
          else if(value==='sort-asc'||value==='sort-desc'){
            const index={price:1,prevClose:2,score:4,volumeRatio:6,delivery:7,obv:8,changeOi:9}[key];
            if(index!==undefined) sortRows(index,value==='sort-asc'?'asc':'desc');
          }
          apply();
        });
      ...
      const width=Math.min(menu.offsetWidth||210,window.innerWidth-16);
    ...
    // Capture only on filter buttons so the legacy inline filter handlers cannot replace this menu.
    ...
    // Bubble phase is intentional: it lets the menu open before outside-click handling runs.
