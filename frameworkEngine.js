...
const frameworkEngine = {
    ...
    processAssetData: function(rawData) {
        ...
        // Risk Penalty Modifier (added Alpha 1.6): applied AFTER the 5-pillar
        // weighted score, not as a 6th independent pillar. Risk's own formula
        // reuses Debt/Equity, ROE, growth, RSI and ADX - the same raw inputs
        // already scored inside Fundamental and Technical. Adding it as a
        // separate weighted vote would double-count those same numbers a
        // second (or third) time. Instead: only HIGH risk (a real tail-risk
        // signal, not routine variation) pulls the final score down - this
        // catches the case where a single severe risk factor is hiding
        // inside an otherwise-fine weighted average, without inflating the
        // score for every stock based on numbers already counted elsewhere.
        let finalScore = scoreMatrix.score;
        let finalRating = scoreMatrix.rating;
        let riskPenaltyApplied = 0;
        if (finalScore !== null && riskMatrix.riskScore >= 70) {
            // riskScore 70 -> 0 point penalty, riskScore 100 -> 10 point penalty
            riskPenaltyApplied = Math.round(((riskMatrix.riskScore - 70) / 30) * 10);
            if (riskPenaltyApplied > 0) {
                finalScore = Math.max(0, finalScore - riskPenaltyApplied);
                if (window.VIKRAM_SCORE_ENGINE && typeof window.VIKRAM_SCORE_ENGINE.getRating === "function") {
                    finalRating = window.VIKRAM_SCORE_ENGINE.getRating(finalScore);
                }
            }
        }
        ...
        return {
            meta: {
                ...
                preRiskPenaltyScore: scoreMatrix.score,
                riskPenaltyApplied: riskPenaltyApplied
            },
            technical: {
                ...rawData.technical
            },
